// api/server.js  — Express + S3 JSON DB + Auth ฝั่งเซิร์ฟเวอร์ (CommonJS)
...
// --- Static web root (โฟลเดอร์รากโปรเจ็กต์ มี index.html) ---
...
// ===== Auth (users.json เก็บ {username, hash, createdAt}) =====
async function ensureUsersFile(){
  const users = await getJson('users.json', '[]'); // array
  if (!Array.isArray(users)) await putJson('users.json', []);
}
